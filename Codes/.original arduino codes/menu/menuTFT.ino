...
// --- PIN DEFINITIONS ---
// SPI, TFT, and TOUCH pins are defined in TFT_eSPI User_Setup.h!
#define TOUCH_IRQ  1  // Used for Wake-up interrupt
#define SD_CS      5
#define TFT_BL    38  // Backlight Pin (Moved to 38 since 21 is Touch CS)

// Define Serial2 RX/TX just in case your board package doesn't define them
#ifndef RXD2
#define RXD2 16
#endif
#ifndef TXD2
...
void IRAM_ATTR handleTouchInterrupt() {
  if (isAsleep) {
    isAsleep = false; 
    // We handle the actual brightness ramp in the loop or a task
    // as it's not ideal to do long delays in an ISR
  }
}

void setBacklight(int brightness) {
  ledcWrite(pwmChannel, brightness); // 0 to 255
}
...
void setup() {
  ...
  // Initialize TFT_eSPI (it reads pins from User_Setup.h automatically)
  tft.begin();
  tft.setRotation(1);
  tft.setSwapBytes(true); // Required for correct LVGL colors
  tft.fillScreen(TFT_BLACK);

  // Run SPIFFS-based touch calibration UI
  touch_calibrate();
